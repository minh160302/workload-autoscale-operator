...
```sh
make docker-build docker-push IMG=<some-registry>/workloadautoscale-operator:tag
```
...
```sh
make deploy IMG=<some-registry>/workloadautoscale-operator:tag
```
...
```sh
make build-installer IMG=<some-registry>/workloadautoscale-operator:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/workloadautoscale-operator/<tag or branch>/dist/install.yaml
```
